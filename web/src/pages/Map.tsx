...
export default function MapPage() {
  ...
  const [hovered, setHovered] = useState<SiteGeo | null>(null)
  ...
  useEffect(() => {
    api<SiteGeo[]>('/api/map/sites')
      .then(setSites).catch(() => {}).finally(() => setLoading(false))
  }, [])

  const regions = [...new Set(sites.map(s => s.region).filter(Boolean))].sort() as string[]
  const visible = region ? sites.filter(s => s.region === region) : sites
  const bounds  = region ? boundsOf(visible) : FULL_BOUNDS

  const sorted = [...visible].sort((a, b) => {
    const ra = a.worst_severity ? SEV_RANK[a.worst_severity] ?? 99 : 100
    const rb = b.worst_severity ? SEV_RANK[b.worst_severity] ?? 99 : 100
    return rb - ra
  })

  const alarmedCount  = visible.filter(s => s.open_alarms > 0).length
  const criticalCount = visible.filter(s => s.worst_severity === 'critical').length
    ...
    <div>
      ...
      <div style={{ marginBottom: 10, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        <Select
          ...
          allowClear
          style={{ width: 180 }}
          onChange={v => { setRegion(v ?? null); setHovered(null) }}
        ...
        <span style={{ color: '#666', fontSize: 13 }}>
          ...
          {alarmedCount > 0 && <>
            {' · '}
            <span style={{ color: SEV_COLOR.critical, fontWeight: 600 }}>{criticalCount} critical</span>
            {' · '}
            <span style={{ color: SEV_COLOR.major }}>{alarmedCount} with alarms</span>
          </>}
        </span>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 12, fontSize: 12, alignItems: 'center' }}>
          {SEV_LABELS.map(s => (
            <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <svg width={10} height={10}><circle cx={5} cy={5} r={4} fill={SEV_COLOR[s]} /></svg>{s}
            </span>
          ))}
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <svg width={10} height={10}><circle cx={5} cy={5} r={4} fill="#52c41a" opacity={0.5} /></svg>ok
          </span>
        </div>
      </div>

      {/* map */}
      <div ref={containerRef} style={{ border: '1px solid #c8d8e8', borderRadius: 8, overflow: 'hidden', background: '#deeaf4', position: 'relative' }}>
        <svg ref={svgRef} viewBox={`0 0 ${VW} ${VH}`} width="100%"
          ...
          {/* BiH canton paths — transformed from source SVG coordinate space */}
          <g transform={bihTransform(bounds)}>
            {BIH_PATHS.map((d, i) => (
              <path key={i} d={d}
                fill="#e4ede4" stroke="#8aaa8a" strokeWidth={0.8} strokeLinejoin="round" />
            ))}
          </g>

          {/* site dots */}
          {sorted.map(s => {
            const [cx, cy] = project(s.lat, s.lon, bounds)
            const isHov    = hovered?.site_key === s.site_key
            const hasAlarm = s.open_alarms > 0
            const fill     = s.worst_severity ? SEV_COLOR[s.worst_severity] : '#52c41a'
            const r        = hasAlarm ? 5 : 3
            return (
              <circle key={s.site_key}
                cx={cx} cy={cy} r={isHov ? r + 1.5 : r}
                fill={fill} fillOpacity={isHov ? 1 : hasAlarm ? 0.90 : 0.30}
                stroke={isHov ? '#000' : hasAlarm ? fill : '#6a6'}
                strokeWidth={isHov ? 2 : hasAlarm ? 0.6 : 0.2}
                style={{ cursor: 'pointer' }}
                onMouseEnter={() => setHovered(s)}
                onMouseLeave={() => setHovered(null)}
                onClick={() => nav(`/sites/${encodeURIComponent(s.site_key)}`)}
              />
            )
          })}
        </svg>
        ...
        {hovered && tipPos && (
          <div style={{
            ...
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 3 }}>
              {hovered.site_key}
              {hovered.display_name && hovered.display_name !== hovered.site_key &&
                <span style={{ fontWeight: 400, color: '#666', marginLeft: 6 }}>{hovered.display_name}</span>}
            </div>
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 3 }}>
              {hovered.region     && <Tag style={{ margin: 0 }}>{hovered.region}</Tag>}
              {hovered.municipality && <Tag style={{ margin: 0 }} color="default">{hovered.municipality}</Tag>}
              {hovered.technologies?.length > 0 && <Tag style={{ margin: 0 }} color="blue">{hovered.technologies.join('/')}</Tag>}
            </div>
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 2 }}>
              {hovered.has_genset  && <Tag style={{ margin: 0 }} color="purple">genset</Tag>}
              {hovered.has_battery && <Tag style={{ margin: 0 }} color="cyan">battery</Tag>}
              {hovered.has_solar   && <Tag style={{ margin: 0 }} color="gold">solar</Tag>}
              {hovered.open_alarms > 0
                ? <Tag style={{ margin: 0 }} color="red">{hovered.open_alarms} alarm{hovered.open_alarms > 1 ? 's' : ''} · {hovered.worst_severity}</Tag>
                : <Tag style={{ margin: 0 }} color="green">OK</Tag>}
            </div>
